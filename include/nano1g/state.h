...
typedef struct n1g_i2c {
    ...
    uint64_t pcf_adc_deadline;
    uint64_t pcf_adc_conversions;
    uint64_t pcf_low_battery_deadline;
    uint64_t pcf_low_battery_standby_deadline;
    uint64_t pcf_low_battery_events;
    uint64_t rtc_base_ticks;
    uint64_t rtc_last_second;
    uint64_t rtc_second_interrupts;
    uint64_t rtc_alarm_interrupts;
    uint16_t pcf_adc_result1;
    uint16_t pcf_adc_result2;
    n1g_pcf_wake_reason_t pcf_last_wake;
    bool pcf_adc_ready;
    bool pcf_low_battery;
    bool pcf_standby;
...
typedef struct n1g_backlight {
    uint32_t pwm_regs[0x40 / 4];
    uint32_t dimmer_regs[0x100 / 4];
    uint64_t generation;
