...
run_image:
    ...
    if (s.opts.profile == N1G_PROFILE_APPLE) {
        ...
        n1g_info(&s,
                "apple_preferences hits dirty=%llu check=%llu wrapper=%llu writer=%llu open=%llu write=%llu clear_pre=%llu clear_post=%llu sync=%llu disk_writes=%llu,%llu,%llu,%llu,%llu,%llu,%llu,%llu,%llu dirty_bytes=%u,%u,%u,%u,%u,%u,%u,%u,%u",
                ...
                (unsigned long long)s.counters.apple_preferences_disk_writes[0],
                (unsigned long long)s.counters.apple_preferences_disk_writes[1],
                (unsigned long long)s.counters.apple_preferences_disk_writes[2],
                (unsigned long long)s.counters.apple_preferences_disk_writes[3],
                (unsigned long long)s.counters.apple_preferences_disk_writes[4],
                (unsigned long long)s.counters.apple_preferences_disk_writes[5],
                (unsigned long long)s.counters.apple_preferences_disk_writes[6],
                (unsigned long long)s.counters.apple_preferences_disk_writes[7],
                (unsigned long long)s.counters.apple_preferences_disk_writes[8],
                s.counters.apple_preferences_last[0][7],
                s.counters.apple_preferences_last[1][7],
                s.counters.apple_preferences_last[2][7],
                s.counters.apple_preferences_last[3][7],
                s.counters.apple_preferences_last[4][7],
                s.counters.apple_preferences_last[5][7],
                s.counters.apple_preferences_last[6][7],
                s.counters.apple_preferences_last[7][7],
                s.counters.apple_preferences_last[8][7]);
        n1g_info(&s,
                "apple_power hits handler=%llu gesture=%llu classify=%llu filter=%llu menu_key=%llu idle=%llu power_timeout=%llu transition=%llu ui_shutdown=%llu pwrp_entry=%llu pwrp_handle=%llu display_arm=%llu display_callback=%llu timer_dispatch=%llu display_dispatch=%llu transition7=%llu predicate=%llu media=%llu global=%llu false=%llu true=%llu sm_dispatch=%llu sm_result=%llu prefs_guard=%llu gate_result=%llu checker_call=%llu gate_query=%llu gate_response=%llu gate_release=%llu gate_init=%llu pwrp_post=%llu pwrp_broadcast=%llu pwrp_count=%llu pwrp_callback=%llu guard_values=0x%08x,0x%08x,0x%08x gate_values=%u,%u,%u pwrp_values=0x%08x,0x%08x,0x%08x,%u,0x%08x event_types=%u,%u,%u predicate_values=0x%08x,0x%08x,%u,%u sm_values=%u,0x%08x,0x%08x,%u,0x%08x,0x%08x",
